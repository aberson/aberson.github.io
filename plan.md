...
- **Depends on:** 3
- **Status:** DONE (2026-07-19) — `CaseStudyLayout.astro` (wraps BaseLayout, single `<h1>`) + `src/pages/work/[slug].astro` (featured-only `getStaticPaths`, typed `CollectionEntry<'projects'>`). alpha4gate (1066w) + toybox (1071w) case studies authored Problem→Approach→Outcome from the Career master profile; honesty/metric-fidelity verified against the primary source by review (no invented numbers; finance-by-analogy framing; solo/engineering-rigor caveats). Featured cards link `/work/<slug>/` (trailing slash = no redirect); non-featured expose no link. Verified in Playwright (1 h1/page, no overflow, light/dark/mobile). Reviewers PASS (iter 2). build/check/lint 0.
...
- **Depends on:** 4
- **Status:** DONE (2026-07-19) — `BaseHead` (per-page canonical + OpenGraph + favicon), `ResumeSection` (Download-PDF → valid placeholder `public/resume.pdf` + last-updated), `ContactSection` (`mailto:aberobison@gmail.com` + LinkedIn). Operator provided the two pending values (email + LinkedIn `.../abraham-robison-7183982a/`); `consts.ts` resolved and the `TODO:` placeholder gating removed (`grep -r "TODO:" src/` now empty — unblocks Step 6). Bonus: **About section filled** with factual bio from the profile README (flagged as a scope addition beyond the literal step). a11y (axe) 0 serious/0 critical on hub + case-study; `check:links` (linkinator, `linkedin.com` [999 anti-bot] + pre-deploy origin skipped) 15 links 0 broken; `check:a11y` script fails-loud on a non-2xx page load. Reviewers PASS (iter 2). build/check/lint 0.
